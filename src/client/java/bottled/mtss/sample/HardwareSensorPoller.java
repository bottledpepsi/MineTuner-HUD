...
public final class HardwareSensorPoller {
    ...
    /** Call after saving a config change to { hardwareSensorsEnabled}, {. */
    public static synchronized void reconcileWithConfig() {
        ...
        if (RUNNING == null) {
            ...
            // the running poller's HttpClient no longer matches config.
    ...
    private static Double parseLeadingNumber(String raw) {
        ...
        while (i < len && (Character.isDigit(trimmed.charAt(i)) || trimmed.charAt(i) == '.' || trimmed.charAt(i) == ',')) {
            ...
            // Normalize a comma decimal separator to '.'.
    ...
    private void runLoop() {
        while (!stopRequested) {
            ...
            try {
                ...
                // HttpClient at construction ( reconcileWithConfig).
    ...
    private void pollOnce() {
        ...
        try {
            ...
            // Malformed base URL from the config file.
        ...
        try {
            ...
            // Malformed/unexpected JSON shape.
    ...
    /** Breadth-first walk of the tree (iterative, not recursive. */
