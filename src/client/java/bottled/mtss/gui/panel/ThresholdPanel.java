...
public final class ThresholdPanel {
    ...
    private static boolean isHigherBetter(MtssConfig.Stat stat) {
        return StatRegistry.get(stat).higherIsBetter();
    }

    private static float thresholdStep(MtssConfig.Stat stat) {
     ...
     * Shared by {@link #render}, {@link #isInside} (via {@link #panelHeight}),
     * and {@link #handleClick} so row math can't drift between paint and hit-test.
     */
    ...
    public static void render(GuiGraphicsExtractor g, net.minecraft.client.gui.Font font,
                              ...
                              MtssConfig.ThresholdSettings ts) {
     ...
     */
    private static void clampThresholdOrder(MtssConfig.ThresholdSettings ts, boolean higherBetter, boolean adjustedGood) {
        if (higherBetter) {
            if (adjustedGood) {
    ...
    /** Returns true if the click landed on the Back row (caller should return to the stat settings panel). */
