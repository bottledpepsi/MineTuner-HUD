...
public final class ColorScalePanel {
     ...
     * Returns true if the click landed on the Back row (caller should
     * return to the Appearance panel), false otherwise (including no hit).
     */
    public static boolean handleClick(int mx, int my, int menuX, int menuY, int screenW, int screenH,
                                      MtssConfig.StatListConfig lc, MtssConfig root) {
        int panelH = panelHeight();
        int px = PanelChrome.clampX(menuX, PANEL_W, screenW);
        int py = PanelChrome.clampY(menuY, panelH, screenH);
        ...
        if (PanelChrome.isHoveringRow(mx, my, px, ry0, PANEL_W, ROW_H)) {
            ...
            root.save();
        } else if (PanelChrome.isHoveringRow(mx, my, px, ry3, PANEL_W, ROW_H)) {
            return true; // Color/Scale nests inside Appearance, so Back returns there
        }
        return false;
    }
}
