...
public final class StatSettingsPanel {
    ...
    public static boolean supportsDecimals(MineTunerConfig.Stat stat) {
        return StatRegistry.get(stat).supportsDecimals();
    }

    /** Stats that can be rendered as a rolling graph instead of text. */
    public static boolean supportsGraph(MineTunerConfig.Stat stat) {
    ...
    public static boolean supportsThresholds(MineTunerConfig.Stat stat) {
        return StatRegistry.get(stat).supportsThreshold();
    }

    /** Row count for the stat settings panel. */
    public static int panelRows(MineTunerConfig.Stat stat) {
    ...
    public static int panelHeight(MineTunerConfig.Stat stat) {
    ...
    public static int[] rowOffsets(MineTunerConfig.Stat stat, int py) {
    ...
    public static void render(GuiGraphicsExtractor g, net.minecraft.client.gui.Font font,
        ...
        MineTunerConfig.StatSettings ss = lc.getStatSettings(statSettingsStat);
        ...
        // Header.
        g.text(font, "§e" + I18n.get("gui.minetuner.stat_settings.title", statLabel),
        ...
        String prefixToggle = I18n.get("gui.minetuner.stat_settings.show_prefix")
                + (ss.showPrefix ? " §a" + I18n.get("gui.minetuner.menu.on")
        ...
        if (decimalsRow) {
            ...
            g.text(font, "§f- " + I18n.get("gui.minetuner.stat_settings.decimals", ss.decimals),
        ...
        if (graphRow) {
            ...
            String graphToggle = I18n.get("gui.minetuner.stat_settings.render_as_graph")
                    + (ss.renderAsGraph ? " §a" + I18n.get("gui.minetuner.menu.on")
                    : " §c" + I18n.get("gui.minetuner.menu.off"));
        ...
        if (thresholdsRow) {
            ...
            g.text(font, "§f" + I18n.get("gui.minetuner.stat_settings.custom_thresholds"),
    ...
    public static boolean isInside(int mx, int my, int menuX, int menuY, int screenW, int screenH,
                                   MineTunerConfig.Stat statSettingsStat) {
