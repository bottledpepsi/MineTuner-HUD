...
import bottled.minetuner.config.MineTunerConfig.Stat;
...
public final class TemplateEngine {
    ...
    public static List<List<Token>> getParsedLines(MineTunerConfig.StatListConfig cfg) {
    ...
    private static void warnOnce(int listId, String badBody) {
        ...
        if (warned.add(badBody)) {
            ...
            MineTunerMod.LOGGER.warn("Template list {}: unrecognized token \"{}\" — rendering it as "
    ...
    private static String renderStat(StatToken token) {
        ...
        // Strip it the same way MineTunerRenderer does for showPrefix=off, so a
    ...
    /** Keyed per list (by {@code MineTunerConfig.StatListConfig.id}), so parsing happens
